...
def shift_schedule_today(day, shift):
    ...
    start_time_sec, end_time_sec, minimum_hour = 0, 0, "09:00"
...
def work_record_export(request):
    # Safely parse month and year parameters
    month_str = request.GET.get("month", "").strip()
    year_str = request.GET.get("year", "").strip()

    try:
        month = int(month_str) if month_str.isdigit() else date.today().month
    except ValueError:
        month = date.today().month
